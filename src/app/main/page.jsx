...
export default function Page() {
    ...
    const { savingState: { isSaving, hasError } } = useSavingState();
        ...
        const unsubGoals = onSnapshot(
            ...
            snap => {
                setGoals(prev => {
                    ...
                    const preserveGoals = prev.filter(p =>
                        !newGoals.some(n => n.id === p.id)
                    );
    ...
    const semesterStatuses = useMemo(() => {
        ...
        const allBlocks = goals.flatMap(g => g.buildingBlocks || []).filter(block => block.text?.trim());;
