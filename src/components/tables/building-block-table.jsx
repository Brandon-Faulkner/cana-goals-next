...
const useDebouncedTextUpdater = (semesterId, goalId) => {
    return useCallback(
        debounce((id, text) => {
            toast.promise(
                updateBuildingBlockText(semesterId, goalId, id, text),
                {
...
export function BuildingBlockTable({ goal, semesterId, expanded, toggleGoalExpanded, initialDueDate }) {
            ...
            <Table>
                <TableHeader>
                    <TableRow className="bg-muted/70">
                        <TableHead className="w-1/2 font-medium text-base">Building Blocks</TableHead>
                        <TableHead className="w-1/4 font-medium text-base">Due Date</TableHead>
                        <TableHead className="w-1/4 font-medium text-base">Status</TableHead>
                    </TableRow>
                </TableHeader>
                <TableBody>
                    {goal.buildingBlocks.length > 0 ? (
                        goal.buildingBlocks.map(block => (
                            <BuildingBlockRow
                                ...
                                updateBuildingBlockDueDate={date => handleAction(
                                    updateBuildingBlockDueDate(semesterId, goal.id, block.id, date),
                                    { loading: 'Saving date…', success: 'Saved ✓', error: 'Failed' }
                                )}
                                updateBuildingBlockStatus={status => handleAction(
                                    updateBuildingBlockStatus(semesterId, goal.id, block.id, status),
                                    { loading: 'Saving status…', success: 'Saved ✓', error: 'Failed' }
                                )}
                                deleteBuildingBlock={() => handleAction(
